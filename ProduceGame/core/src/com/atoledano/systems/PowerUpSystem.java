...
public class PowerUpSystem extends IteratingSystem {
    ...
    protected ComponentMapper<PowerUp> powerUpComponentMapper;
    ...
    public PowerUpSystem() {
        super(Aspect.all(PowerUp.class, RigidBody.class, Transform.class, State.class));
    ...
    protected void process(int entityId) {
        PowerUp powerUp = powerUpComponentMapper.get(entityId);
        ...
        switch (powerUp.state) {
            ...
            case MOVING_LEFT:
                if (checkMovable(body, fromV.set(body.getPosition()), toV.set(body.getPosition().x - 0.55f, body.getPosition().y))) {
                    body.setLinearVelocity(-powerUp.speed, 0);
                } else {
                    body.setLinearVelocity(0, 0);
                    body.setTransform(MathUtils.floor(body.getPosition().x) + 0.5f, MathUtils.floor(body.getPosition().y) + 0.5f, 0);
                    powerUp.state = PowerUp.State.NORMAL;
